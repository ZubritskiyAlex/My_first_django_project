...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_link(self, obj):
        uri = reverse('user-detail', kwargs={'username': obj.username})
        return self.context['request'].build_absolute_uri(uri)

    def update(self, instance, validate_data ):
        request = self.context.get('request')
        user = request.user
        if user.is_authenticated and instance.author_id:
            return super(ArticleSerializer, self).update(instance, validate_data)
